...
# --- BANCO DE DADOS DE GANCHOS v1.3 (COM NOVOS GANCHOS E FORMATOS) ---
HOOK_DATABASE = {
    ...
    "Cenários e 'Ponto de Vista' (POV)": [
        ...
        {"text": "POV: Você finalmente [decidiu fazer isso] e agora [você tem isso]", "format": "Reels"},
        ...
        {"text": "POV: Quando você finalmente parou de acreditar [nisso] e agora você [alcançou isso]", "format": "Reels"},
        {"text": "POV: Aquele momento em que você [alcançou isso] e agora você comemora [isso]", "format": "Reels"},
        {"text": "POV: Você finalmente [está fazendo isso] e se sente [assim]", "format": "Reels"},
...
# MUDANÇA: Prompt do Criador (Tab 2) agora pede a "Proposta de Conteúdo"
def create_new_hook(tema, principios, model):
    ...
    **Formato de Resposta (obrigatório):**
    ---
    **1. Gancho:** [Seu primeiro gancho]
    **Proposta de Conteúdo:** [Explicação breve do que abordar no post, qual o ângulo]
    ---
    **2. Gancho:** [Seu segundo gancho]
    **Proposta de Conteúdo:** [Explicação breve do que abordar no post, qual o ângulo]
    ---
    **3. Gancho:** [Seu terceiro gancho]
    **Proposta de Conteúdo:** [Explicação breve do que abordar no post, qual o ângulo]
    """
    try:
...
# MUDANÇA: Adaptador (Tab 3) agora lê a nova estrutura do HOOK_DATABASE
...
with tab1:
    ...
    if categoria_escolhida:
        ...
        for hook in HOOK_DATABASE[categoria_escolhida]:
            ...
            with col1:
                # Exibe o gancho e o formato sugerido
                st.info(hook['text'])
                st.caption(f"Formato Ideal Sugerido: {hook['format']}")
            with col2:
                # O botão de copiar agora é único para cada gancho
                if st.button("Copiar", key=hook['text']):
                    st.toast(f"'{hook['text']}' copiado!")
                    # (Ainda usa o feedback toast, pois a cópia real precisa de JS)
    ...
    if st.button("Gerar Novos Ganchos"):
        if not tema_criador or not principios_escolhidos:
            ...
            with st.spinner(f"Gerando ganchos com base em '{', '.join(principios_escolhidos)}'..."):
                ...
                # A resposta já vem formatada em markdown
